...
flowchart LR
    ...
    Recovery[AnnotationGeometryRecoveryService] -- optional BatchAnchorResolver --> BatchScan
    BatchScan[One Aho-Corasick pass over pending quotes] --> CandidateCache
    QuoteIndex -- warmed or repeated quote --> CandidateCache
...
Both caches are local, disposable, and handle-scoped. Deterministic profiling covers both quote distributions. For 50 stale annotations over 200 pages, one repeated quote falls from 10,000 independent quote-scan page visits to 200 because its candidate list is reused. For 50 distinct stale quotes over the same 200 pages, per-anchor resolution still records 10,000 page-string scan visits and 50 quote-cache misses, while `PdfPageTextCache` bounds underlying page-text extraction misses to 200.

Batch recovery removes that remaining hotspot: `AnnotationGeometryRecoveryService` detects the optional `BatchAnchorResolver` capability, `PdfrxPdfAdapter` warms the handle-scoped index with one automaton pass over all pending quotes, and every per-anchor lookup then hits the candidate cache (measured: one 200-page warm-up, 0 per-anchor quote-scan visits, 50 hits / 0 misses). Warm-up results are byte-for-byte the candidates a per-quote scan finds (including overlaps); candidate verification, context scoring, ambiguity rules, failure eviction, and disposable cache lifetime are unchanged. A failed warm-up changes nothing — each anchor then scans per-quote exactly as before — and per-highlight failure isolation holds in both paths: one broken anchor resolves as unresolved and suppresses only itself.
